...
<body>
    ...
    <div id="buscador">
        <form method="GET" action="anuncio.php" id="search-form">
            <div id="buscador-encima">
                <input id="input-buscador" type="text" name="busqueda" placeholder="Buscar por nombre de artículo"
    ...
    <div id="paginacion">
        <a href="?pagina=<?php echo $paginaActual - 1; ?>"
            class="botonesPagina <?php if ($paginaActual <= 1) echo 'a-disabled'; ?>">← Anterior</a>

        <?php for ($i = 1; $i <= $paginasTotales; $i++): ?>
        <a class="botonesPagina <?php if ($i == $paginaActual) echo 'a-disabled'; ?>"
            href="?pagina=<?php echo $i; ?>"><?php echo $i; ?></a>
        <?php endfor; ?>

        <a href="?pagina=<?php echo $paginaActual + 1; ?>"
            class="botonesPagina <?php if ($paginaActual >= $paginasTotales) echo 'a-disabled'; ?>">Siguiente →</a>
    </div>
